...
import BridgeCallout from '@/components/BridgeCallout';
                        ...
                        {lesson.content.map((section, i) => {
                            ...
                            return (
                                <div key={i} style={{
                                    ...
                                    {section.type === 'interactive-code' && section.codeConfig ? (
                                    ...
                                    ) : section.type === 'interactive' && section.chartConfig ? (
                                        <div className="mt-4">
                                            <div className="mb-4 text-sm text-slate-300">
                                                <Latex block>{sectionBody}</Latex>
                                            </div>
                                            <InteractiveChart config={section.chartConfig} />
                                        </div>
                                    ) : (
                                        <div style={{
                                            fontSize: '0.9rem',
                                        ...
                                        {/* Bridge Callout: use explicit bridge data or smart fallback */}
                    {(() => {
                        const bridgeSection = lesson.content.find(s => s.type === 'bridge');
                        if (bridgeSection?.bridgeData) {
                            return (
                                <BridgeCallout
                                    interview={bridgeSection.bridgeData.interview}
                                    desk={bridgeSection.bridgeData.desk}
                                    practiceLink={bridgeSection.bridgeData.practiceLink}
                                    practiceLabel={bridgeSection.bridgeData.practiceLabel}
                                />
                            );
                        }
                        // Smart fallback
                        return (
                            <BridgeCallout
                                interview={{
                                    fr: `On pourrait vous demander : "Expliquez-moi ${t(lesson.title).toLowerCase()} et donnez un exemple concret."`,
                                    en: `You might be asked: "Explain ${t(lesson.title).toLowerCase()} and give a concrete example."`
                                }}
                                desk={{
                                    fr: 'Visualisez les payoffs et Greeks associés pour mieux comprendre les implications en trading.',
                                    en: 'Visualize the associated payoffs and Greeks to better understand the trading implications.'
                                }}
                                practiceLink="/interview"
                                practiceLabel={{ fr: 'Tester en entretien IA', en: 'Test with AI interview' }}
                            />
                        );
                    })()}
                    </div>
