    ...
    const requestHint = async () => {
        ...
        try {
            const dataContent = await fetchStreamingResponse(
                [
                    { role: "system" as const, content: `You are a helpful interviewer. The candidate is solving: "${qText}". Solution: "${sText}". Give ONE short sentence hint without revealing the answer. Respond in ${locale === 'fr' ? 'French' : 'English'}.` },
                    { role: "user" as const, content: "Give me a subtle hint." }
                ],
                locale,
                'brainteaser'
            );
            setHints(prev => [...prev, dataContent]);
...
Respond entirely in ${locale === 'fr' ? 'French' : 'English'}. Use Markdown for readability.`;
        ...
        try {
            const aiResponse = await fetchStreamingResponse(initialMessages, locale, 'brainteaser');
    ...
    const sendChatMessage = async (content: string) => {
        ...
        setError(null);
        try {
            const aiResponse = await fetchStreamingResponse(newMsgs, locale, 'brainteaser');
