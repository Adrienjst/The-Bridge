    ...
    lessons: [
        ...
        },
        {
            ...
            content: [
                ...
                {
                    type: 'formula',
                    title: { fr: 'Attribution de P&L de Taylor', en: 'Taylor P&L Attribution' },
                    body: {
                        fr: 'Le trader peut expliquer quotidiennement l\'origine exacte de sa P&L en utilisant l\'expansion de Taylor du prix de l\'option par rapport à tous les paramètres du marché :\n\n$$ \\Delta P\\&L \\approx \\Delta \\cdot dS + \\frac{1}{2}\\Gamma \\cdot (dS)^2 + \\nu \\cdot d\\sigma + \\Theta \\cdot dt + \\rho \\cdot dr $$\n\nEn soustrayant le gain de la couverture en delta ($- \\Delta \\cdot dS$), la **P&L non-hedgée** restante est :\n\n$$ \\text{Unhedged P\\&L} = \\frac{1}{2}\\Gamma \\cdot (dS)^2 + \\Theta \\cdot dt + \\nu \\cdot d\\sigma $$\n\nC\'est l\'équation fondamentale du trader d\'options : le profit vient soit de la convexité (Gamma vs Theta), soit d\'un mouvement favorable de la volatilité implicite (Vega).',
                        en: 'The trader can explain the exact origin of their daily P&L using the Taylor expansion of the option price with respect to all market parameters:\n\n$$ \\Delta P\\&L \\approx \\Delta \\cdot dS + \\frac{1}{2}\\Gamma \\cdot (dS)^2 + \\nu \\cdot d\\sigma + \\Theta \\cdot dt + \\rho \\cdot dr $$\n\nSubtracting the delta hedge gain ($- \\Delta \\cdot dS$), the remaining **Unhedged P&L** is:\n\n$$ \\text{Unhedged P\\&L} = \\frac{1}{2}\\Gamma \\cdot (dS)^2 + \\Theta \\cdot dt + \\nu \\cdot d\\sigma $$\n\nThis is the option trader\'s fundamental equation: profit comes either from convexity (Gamma vs Theta), or a favorable movement in implied volatility (Vega).'
                    }
                }
            ]
        },
            ...
            content: [
                {
                    ...
                    body: {
                        fr: 'Les produits structurés ont des profils de Greeks **non-linéaires** et complexes :\n\n**Autocall — Delta** :\n- Spot >> barrière autocall : Δ ≈ 0 (le produit va se rappeler, le prix est fixe)\n- Spot ≈ barrière autocall : Δ très négatif (gamma spot, si le spot monte → rappel → perte du coupon futur pour le hedger)\n- Spot << barrière autocall mais > PDI : Δ modérément négatif\n- Spot ≈ PDI : Δ très négatif (activation du put DI)\n\n**Autocall — Vega & Impact dynamique (Vanna/Volga)** :\n- Typiquement **short vega** : une hausse de vol augmente la probabilité de toucher la PDI → baisse du prix\n- **Vanna** $\\left(\\frac{\\partial \\nu}{\\partial S}\\right)$ : Si le marché baisse vers la PDI, l\'incertitude (Vega) augmente fortement. Le desk se retrouve de plus en plus "short vega" à mesure que le spot chute.\n- **Volga** $\\left(\\frac{\\partial \\nu}{\\partial \\sigma}\\right)$ : La convexité du Vega. Plus la volatilité monte, plus la probabilité du scénario catastrophe (PDI) croît exponentiellement.\n\n**Autocall — Gamma** :\n- Des « pics » de gamma aux dates d\'observation, en particulier quand le spot est proche des barrières\n- Gap risk : entre deux observations, le gamma s\'accumule\n\n**Implications pour le trading desk** :\n- L\'autocall book est typiquement short vega, short correlation, et expose au risque de « pinning ».\n- Le Vanna aggrave les situations de crise : quand le marché chute, la volatilité monte, et le desk a besoin de racheter encore plus de volatilité pour se couvrir, ce qui amplifie le mouvement (feedback loop).',
                        en: 'Structured products have **non-linear** and complex Greeks profiles:\n\n**Autocall — Delta**:\n- Spot >> autocall barrier: Δ ≈ 0 (product will be called, price is fixed)\n- Spot ≈ autocall barrier: very negative Δ (spot gamma, if spot rises → recall → loss of future coupon for hedger)\n- Spot << autocall barrier but > PDI: moderately negative Δ\n- Spot ≈ PDI: very negative Δ (DI put activation)\n\n**Autocall — Vega & Dynamic Impact (Vanna/Volga)**:\n- Typically **short vega**: vol increase raises PDI probability → price decrease\n- **Vanna** $\\left(\\frac{\\partial \\nu}{\\partial S}\\right)$: If the market drops towards the PDI, uncertainty (Vega) increases sharply. The desk becomes increasingly "short vega" as the spot falls.\n- **Volga** $\\left(\\frac{\\partial \\nu}{\\partial \\sigma}\\right)$: The convexity of Vega. The higher the volatility gets, the more the probability of the worst-case scenario (PDI) grows exponentially.\n\n**Autocall — Gamma**:\n- Gamma "spikes" at observation dates, especially when spot is near barriers\n- Gap risk: between observations, gamma accumulates\n\n**Implications for trading desk**:\n- The autocall book is typically short vega, short correlation, and exposed to "pinning" risk.\n- Vanna worsens crisis situations: when the market falls, volatility rises, and the desk needs to buy back even more volatility to hedge, creating an amplifying feedback loop.'
                    }
                }
            ]
        },
